...
export default function MainContainer() {
  ...
  const askAI = async () => {
    if (question === "") {
      ...
      setQuestion("");
      try {
        const response = await fetch("http://localhost:5555/get_answer", {
          method: "POST",
          headers: {
        ...
        <div className=" mx-2 px-md-5 py-sm-4 chat-scroll">
          {chat.length > 0 ? (
            chat.map((msg) => (
              <Stack direction="vertical" gap={3}>
                ...
                >
                  <div>
                    {msg.isBot ? (
                      <span style={{ fontSize: "12px" }}>HelpGPT</span>
                    ) : (
            ...
            <Stack
              ...
              style={{ color: "black" }}
            >
              <h3 className="mt-sm-5" style={{ color: "#9b9b9b" }}>
                <small>HelpGPT, powered by Pinecone, LangChain, and OpenAI</small>
              </h3>
            </Stack>
          )}
